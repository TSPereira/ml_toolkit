        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if state['step'] == buffered[0]:
                    ...
                    if N_sma >= 5:
                        step_size = group['lr'] * math.sqrt((1 - beta2_t) * (N_sma - 4) /
                                                            (N_sma_max - 4) * (N_sma - 2) / N_sma * N_sma_max /
...
class RAdam4Step(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0, update_all=False,
        ...
        self.buffer = [[None, None] for _ in range(10)]
        super(RAdam4Step, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(RAdam4Step, self).__setstate__(state)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['step'] = -4 if self.additional_four else 0
                    state['exp_avg'] = torch.zeros_like(p_data_fp32)
                    state['exp_avg_sq'] = torch.zeros_like(p_data_fp32)
                else:
                    state['exp_avg'] = state['exp_avg'].type_as(p_data_fp32)
                ...
                if state['step'] > 0:
                    # since this exp requires exactly 4 step, it is hard coded
                    state_step = state['step'] + 4 if self.additional_four else state['step']
                    ...
                    if state_step == buffered[0]:
                        ...
                        if state['step'] > 4:   # since this exp requires exactly 4 step, it is hard coded
                            N_sma_max = 2 / (1 - beta2) - 1
                            N_sma = N_sma_max - 2 * state['step'] * beta2_t / (1 - beta2_t)
                            step_size = group['lr'] * math.sqrt((N_sma - 4) / (N_sma_max - 4) * (N_sma - 2) / N_sma *
                                                                N_sma_max / (N_sma_max - 2)) / (1 - beta1 ** state_step)
