...
from ..utils.log_utils import printv, print_progress_bar
...
def encode_training(data, method='auto', cont_transformer='MinMaxScaler', gamma=None, calc_gamma=False,
    ...
    :param string return_as: format to return encoded features. default: 'sparse'. options: {'sparse', 'dataframe'}
    ...
    for idx, feat in enumerate(cont_feat):
        print_progress_bar(idx + 1, len(cont_feat), prefix='Encoding continuous features: ',
                           suffix=f'| Encoding {feat}',
    ...
    for idx, feat in enumerate(cat_feat):
        print_progress_bar(idx + 1, len(cat_feat), prefix='Encoding categorical features: ',
                           suffix=f'| Encoding {feat}',
                           level=2, verbose=verbose)
