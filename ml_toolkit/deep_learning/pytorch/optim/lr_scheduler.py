...
# For torch versions older than 1.4.0 use the UserDefined _LRScheduler (_LRScheduler of older versions) as base class.
...
class DummyLR(_LRScheduler):
    def __init__(self, optimizer, lr, last_epoch=-1):
        ...
        max_lrs = self._format_param('max_lr', self.optimizer, lr)
        if last_epoch == -1:
            for idx, group in enumerate(self.optimizer.param_groups):
                group['initial_lr'] = max_lrs[idx]
                group['max_lr'] = max_lrs[idx]
                group['min_lr'] = max_lrs[idx]

        super(DummyLR, self).__init__(optimizer, last_epoch)

    @staticmethod
    def _format_param(name, optimizer, param):
        return _format_param(name, optimizer, param)

    def get_lr(self):
        return [group['max_lr'] for group in self.optimizer.param_groups]


class LinearLR(_LRScheduler):
    def __init__(self, optimizer, end_lr, n_steps, last_epoch=-1):
        self.end_lr = end_lr
    ...
    def get_lr(self):
        return [base_lr + (self.end_lr - base_lr) * self.last_epoch for base_lr in self.base_lrs]
...
class OneCycleLR(_LRScheduler):
    ...
    def __init__(self,
        ...
        if version.parse(torch.__version__) >= version.parse('1.4.0'):
            warnings.warn('OneCycleLR was introduced in torch package on version 1.4.0. Use torch version instead!',
...
class DelayerScheduler(_LRScheduler):
    ...
    def __init__(self, optimizer, delay_epochs, after_scheduler, last_epoch=-1):
        ...
        self.finished = False
        super(DelayerScheduler, self).__init__(optimizer, last_epoch)
...
def make_delayed_scheduler(optimizer, base_scheduler, delay_epochs):
